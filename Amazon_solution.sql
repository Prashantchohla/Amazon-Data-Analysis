...
SELECT 
		...
		DENSE_RANK() OVER( ORDER BY SUM(total_sales - (p.cogs * oi.quantity))/sum(total_sales) * 100 DESC) as product_ranking
FROM order_items as oi
JOIN products as p
ON oi.product_id = p.product_id
GROUP BY 1;
...
*/
SELECT 
Cus_full_name as customers,
total_orders,
total_returned,
CASE
	WHEN total_returned > 5 THEN 'Returning_customers' ELSE 'New'
END as cx_category
FROM
(
SELECT 
	c.customer_id,
	CONCAT(c.first_name, ' ', c.last_name) as Cus_full_name,
